...
/**
 * Simulator Controller (Top-level for routing compatibility)
 * 
...
class Simulator extends CI_Controller
{
    public function __construct()
    {
        ...
        // Debug logging
        log_message('info', 'Simulator access attempt - Role: ' . $role_name);
        
        if (!in_array($role_name, $allowed_roles)) {
            log_message('error', 'Simulator access denied - Role not allowed: ' . $role_name);
    ...
    public function get_shift_records($shift_id)
    {
        ...
        try {
            ...
            if (!$this->db->table_exists('production_records')) {
                $this->output
                    ...
                    ->set_output(json_encode([
                        ...
                        'summary' => null,
                        'count' => 0,
                        'message' => 'Bảng production_records chưa tồn tại. Vui lòng chạy migration 016.'
                    ]));
                return;
            }
            ...
            log_message('error', 'get_shift_records error: ' . $e->getMessage());
            $this->output
                ->set_content_type('application/json')
                ->set_status_header(500)
                ->set_output(json_encode([
