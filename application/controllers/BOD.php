 ...
 * @author Do Cong Danh
 * @date 2025-12-10 (Optimized by Gemini)
 */
class BOD extends CI_Controller
{
    ...
    public function customer($action = 'index', $id = null)
    {
        ...
        switch ($action) {
            ...
            case 'delete':
                ...
                break;
            default:
                $data['data'] = $this->CustomerModel->getAllCustomers();
    ...
    public function product($action = 'index', $id = null)
    {
        ...
        switch ($action) {
            ...
            case 'delete':
                ...
                break;
            default:
                $data['data'] = $this->ProductModel->getAllProducts();
    ...
    public function updateProduct()
    {
        ...
        try {
            ...
            if ($success) {
                $refresh_stats = $this->OrderModel->refreshAllWarnings($productId);
                $success_message = 'Cập nhật sản phẩm thành công.' . sprintf(' (Đã làm mới %d/%d đơn hàng liên quan).', $refresh_stats['success'], $refresh_stats['total']);
                $this->session->set_flashdata('success_js', json_encode(['title' => 'Thành công!', 'message' => $success_message]));
                redirect(site_url('BOD/product') . '?msg=success');
            } else {
    ...
    public function project($action = 'index', $id = null)
    {
        ...
        switch ($action) {
            ...
            case 'deleteproject':
                ...
                break;
            default:
                $data['data'] = $this->OrderModel->getAllOrders();
