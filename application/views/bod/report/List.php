	...
	<div class="container-fluid py-1 px-3">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
				...
				<li class="breadcrumb-item text-sm text-dark active" aria-current="page"><?= lang('breadcrumb_reports'); ?></li>
			</ol>
			<h6 class="font-weight-bolder mb-0"><?= lang('breadcrumb_reports'); ?></h6>
		</nav>
	...
	<div class="card-header p-0 w-75 position-fixed mt-n4 mx-2 z-index-2">
		<div class="shadow-dark border-radius-lg d-flex px-5 pt-4 pb-3">
			<div class="col-8 d-flex align-items-center">  
				<i class="material-icons pr-3">list</i>
				<h6 class="mb-0">Danh sách đơn hàng</h6>
			</div>
			<div class="col-4 text-end">
				<!-- empty: no add button for report listing -->
			</div>
		</div>
	</div>
</div>
	...
	<div class="row">
		<div class="card">
			...
			<div class="table-responsive p-0">
				...
				<tbody>
					<?php if (!empty($projects)) : $i = 1; foreach ( array_reverse($projects) as $value) : ?>
						<tr>
					<td>
					...
					<td>
						<span class="text-sm font-weight-bold pl-3"><?= htmlspecialchars($value->project_name ?? '-', ENT_QUOTES) ?></span>
					</td>
					<td>
						<span class="text-sm font-weight-bold pl-3"><?= htmlspecialchars($value->cust_name ?? '-', ENT_QUOTES) ?></span>
					</td>
					<td>
