...
if (removeBtn) {
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Thay đổi logic validation BOM cho phù hợp với cấu trúc mới (không còn dùng checkbox trên từng dòng)
        // Thay vào đó, kiểm tra các custom material rows hoặc các hàng hiện có đã được điền.
        
        // Lấy tất cả các dòng BOM hiện đang được hiển thị trong container
        const allBomRows = bomContainer.querySelectorAll('.bom-row');
        let validBomEntryFound = false;
        let hasError = false;

        if (allBomRows.length === 0) {
            e.preventDefault();
            alert('⚠️ Bắt buộc thêm ít nhất 1 nguyên liệu cho sản phẩm!');
                    return false;
                }
            
        allBomRows.forEach(row => {
            const materialIdInput = row.querySelector('.material-id-input'); // Đối với material đã tồn tại
            const materialNameInput = row.querySelector('input[name="bom_material_names[]"]'); // Đối với custom material
            const quantityInput = row.querySelector('input[name="bom_quantities[]"]');
            const uomInput = row.querySelector('input[name="bom_uoms[]"]'); // Sửa tên biến

            // Kiểm tra xem dòng BOM có dữ liệu hợp lệ không
            ...
            // Nếu có tên vật liệu hoặc ID vật liệu đã chọn, và số lượng > 0
            if ((materialIdInput.value.trim() || materialName) && quantity > 0 && uom) {
                validBomEntryFound = true;
            } else if (materialName || (quantity > 0) || uom) {
                // Nếu có bất kỳ trường nào được điền nhưng không đầy đủ, báo lỗi
                e.preventDefault();
                alert('Vui lòng điền đầy đủ Tên nguyên liệu, Số lượng/SP > 0 và Đơn vị cho tất cả các dòng BOM đã thêm!');
                if (materialNameInput) materialNameInput.focus();
                else if (quantityInput) quantityInput.focus();
                else if (uomInput) uomInput.focus();
                hasError = true;
                return; // Thoát khỏi forEach
            }
        });

        if (hasError) return false;

        if (!validBomEntryFound) {
            e.preventDefault();
            alert('⚠️ Bắt buộc thêm ít nhất 1 nguyên liệu có định mức > 0 cho sản phẩm!');
            return false;
        }
        
        // Remove disabled inputs from submission (chỉ dành cho các checkbox cũ, nay không còn nữa)
        // Logic mới: chỉ gửi đi các hàng BOM hợp lệ. Các hàng không hợp lệ sẽ bị bỏ qua hoặc bị chặn bởi validation trên.
